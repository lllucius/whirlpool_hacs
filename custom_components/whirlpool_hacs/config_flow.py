...
async def validate_input(hass: HomeAssistant, data: dict[str, str]) -> dict[str, str]:
    ...
    if not appliances_manager.get_appliances():
        raise NoAppliances

    return {"title": f"{data[CONF_REGION]}.{data[CONF_BRAND]}.{data[CONF_USERNAME]}"}
...
class WhirlpoolConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None) -> ConfigFlowResult:
        ...
        try:
            ...
            await self.async_set_unique_id(
                self._get_unique_id(user_input), raise_on_progress=False
            )
    ...
    def _get_unique_id(self, user_input: dict[str, Any]):
        uid = f"{user_input[CONF_REGION]}.{user_input[CONF_BRAND]}.{user_input[CONF_USERNAME]}"
        return uid.lower()
    ...
    async def async_step_reconfigure(self, user_input: dict[str, Any] | None = None):
        ...
        try:
            ...
            data = {CONF_USERNAME: user_input[CONF_USERNAME],
                    ...
                    CONF_REGION: user_input[CONF_PASSWORD],
                    CONF_BRAND: user_input[CONF_PASSWORD]}
            return self.async_update_reload_and_abort(
                self._get_reconfigure_entry(),
                data_updates=data,
            )

        # Reprompt for users attention
        return self.async_show_form(
            step_id="user", data_schema=STEP_USER_DATA_SCHEMA
        )
